    ...
    {
        {
            ...
            while (vending)
            {
                ...
                while (selecting)
                {
                    ...
                    if (Vend.selectSlot(selection) && Vend.getSelectedSlotQuantity(selection) > 0)
                    {
                        // Check if user has enough money to buy selected product
                        if (Vend.getSelectedSlotPrice(selection) < Money.getBalance())
                        {
                            // Decrease balance by selected product price
                            Money.takeFromBalance(Vend.getSelectedSlotPrice(selection));
                            // Send log information to logger
                            Log.writeToFile(Money.getLastEvent());
                            // Dispense product
                            Vend.dispenseSelectedSlot(selection);
                            // Send log information to logger
                    ...
                    else if (!Vend.selectSlot(selection))
                    {
                        ...
                        Ui.setDisplayToUser("Invalid ID");
                        // Send log information to logger
                        Log.writeToFile(Ui.getLastEvent());
                    }
                    // Process if selected slot is out of stock
                    else if (Vend.getSelectedSlotQuantity(selection) < 1)
                    {
                        // Display to user "Out of Stock"
                        Ui.setDisplayToUser("Out of Stock");
                        // Send log information to logger
                        Log.writeToFile(Ui.getLastEvent());
                    }
                }
